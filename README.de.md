...
| `log_directory` | Verzeichnis fuer die Installer-Logdatei `oak-installer.log`. MUSS ausserhalb des installierten Projekts liegen, damit Cache-Leerungen das Log niemals loeschen. Absoluter Pfad oder relativ zum Installer-Root. Leer = Default `<installer-root>/logs`. |
...
Der Installer schreibt ein strukturiertes, append-only Audit-Log `oak-installer.log`. Das Logverzeichnis wird einmal pro Request aufgeloest und liegt bewusst **ausserhalb des installierten Projekts** (damit ausserhalb des `<target>/var/`-Cachebaums). So wird das Log durch Cache-Leeren, Erstinstallations-Cleanup oder aehnliche Vorgaenge niemals geloescht:

1. `log_directory` in `config.php` falls gesetzt (absoluter Pfad oder relativ zum Installer-Root).
2. Default: `<installer-root>/logs` (ein `logs`-Ordner neben dem installer-eigenen `src/`).

Zusaetzliches Safety-Net: `clearCacheDirectory()` verschont jedes Top-Level-`log`-/`logs`-Verzeichnis innerhalb des Cachebaums, sodass auch Laufzeit-Logs (z. B. Symfony `var/log`) ein Cache-Leeren ueberstehen. Logging ist best-effort und bringt niemals eine Installation zum Scheitern. Implementierung: [`src/app/FilesystemSupport.php`](src/app/FilesystemSupport.php).
